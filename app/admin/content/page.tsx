...
export default function ContentManager() {
    ...
    async function submit(event: FormEvent<HTMLFormElement>) {
        ...
        const form = new FormData(event.currentTarget); const files = Array.from(form.getAll("images")).filter((file): file is File => file instanceof File && file.size > 0).slice(0, 5);
    ...
    return <div className="wrap"><span className="eyebrow">Content manager</span><h1 className="display big-title">Publish your work.</h1><p className="muted">Add products, projects, services or up to five gallery images. Only published content appears on the website.</p><form className="form card content-form" onSubmit={submit}><label>Content type<select value={kind} onChange={(e) => setKind(e.target.value)}>{kinds.map((item) => <option key={item}>{item}</option>)}</select></label><label>Title<input name="title" required /></label><label>Category<input name="category" placeholder="Steel, Windows, Kitchen..." /></label><label>Description / service items<input name="description" placeholder="Use commas between service items" /></label><label>Images <span className="muted">(up to 5)</span><input name="images" type="file" accept="image/jpeg,image/png,image/webp" multiple /></label><button className="btn primary" type="submit">Publish content</button>{status && <p role="status" className="eyebrow">{status}</p>}</form></div>;
}